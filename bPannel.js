...
function init_te_usetraffic(){
    var te_usetraffic = echarts.init(document.getElementById('te_usetraffic'));
    $.ajax({
        type: "GET",
        url: "/api/tertraffic/usetraffic",
        datatype: "JSON",
        success: function (data) {
            var tr_data = data.data;
            var x_data = new Array();
            var y_data = new Array();
            for ( x in tr_data){
                x_data.push(tr_data[x].usetraffic);
            }
            for ( y in tr_data){
                y_data.push(tr_data[y].lasttime);
            }
            option = {
                tooltip: {
                    trigger: 'axis',
            ...
            te_usetraffic.setOption(option);
        }
    });
}
